...
using System.Text;
using ChatBot.bot.services.translator.data.clients.vk.response;
using Newtonsoft.Json;

namespace ChatBot.bot.services.translator.data.clients.vk;
...
public class VkTranslateClient {
    ...
    public async Task<string[]?> Translate(string text, string lang, string? sourceLang = null, EventHandler<string>? callback = null) {
        try {
            ...
            callback?.Invoke(this, $"Exception while translating via vk: {e.Data}"); 
            return null;
        }
    }
}