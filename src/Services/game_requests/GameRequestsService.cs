...
public class GameRequestsService : Service {
    ...
    public ErrorCode AppendRequest(GameRequest request, ChatMessage message) {
        if (Options.State == State.Disabled) return ErrorCode.ServiceDisabled;
        if (Options.GameRequestsPoint!.TryGetValue(message.UserId, out var value) && value <= 0) return ErrorCode.TooFewPoints;
        if (Options.GameRequestsSet!.Contains(request.GameName.GetHashCode())) return ErrorCode.AlreadyContains;
    ...
    public ErrorCode RemoveRequestAt(int index, ChatMessage message) {
        if (Options.State == State.Disabled) return ErrorCode.ServiceDisabled;
        if (!PermissionHandler.Handle(Permission.Dev, message)) return ErrorCode.PermDeny;
        if (index >= Options.GameRequests?.Count) return ErrorCode.WrongInput;
