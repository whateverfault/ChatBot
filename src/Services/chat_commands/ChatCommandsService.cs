...
public class ChatCommandsService : Service {
    ...
    private string?[] ProcessArgs(List<string?> args) 
    {
        ...
        switch (args.Command.CommandText) {
            ...
            case "gr-take-point": {
                ...
                Client.SendReply(args.Command.ChatMessage.Channel, args.Command.ChatMessage.Id, $"Очко забрано у пользователя '{commandArgs[0]}'.");
            ...
            case "mr-guess": {
                ...
                if (commandArgs[0] != message!.Username) {
                    ...
                    Client.SendReply(args.Command.ChatMessage.Channel, args.Command.ChatMessage.Id, $"Правильно, это было сообщение от {message.Username}.");
            ...
            #endregion
            default: {
                Client.SendReply(args.Command.ChatMessage.Channel, args.Command.ChatMessage.Id, $"Неизвестная комманда: {Options.CommandIdentifier}{args.Command.CommandText}");
                break;
            }
        }
    }
    ...
    private void SendUsage(OnChatCommandReceivedArgs args) {
        const string usage = "~<комманда> \"аргумент1\" \"аргумент2\" ...";
    ...
    private void SendCmds(OnChatCommandReceivedArgs args, string[] commandArgs) {
        var cmds = new[] {
                             ...
                             "3. ~gr-add <game_name> - добавить заказ игры. ",
                             "4. ~gr-rem <game_position> - удалить заказ игры. ",
                             "5. ~gr-list - список заказов игр. ",
                             "6. ~gr-give-point <nickname> - выдать очко.",
                             "7. ~gr-take-point <nickname> - забрать очко.",
                             "8. ~gr-enable - включить заказы.",
                             "9. ~gr-disable - отключить заказы.",
                             "10. ~mr-guess <nick_name> - угадать ник написавшего сообщение",
                             "11. ~mr-whose - вывести ник написавшего сообщение",
                         };
